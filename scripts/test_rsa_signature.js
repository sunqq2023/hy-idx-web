 ...
 * RSA签名测试脚本 (Node.js)
 * 使用方法：
 *   node scripts/test_rsa_signature.js <接口名称> [参数...]
 *
 * 示例：
 *   node scripts/test_rsa_signature.js getMix --phone 18628395611 --address 0xF5C1d985C52aE8cB0Ab7a642E378089eF15D9300
 *   node scripts/test_rsa_signature.js addMix --phone 18628395611 --address 0xF5C1d985C52aE8cB0Ab7a642E378089eF15D9300 --amount 10.5
 *   node scripts/test_rsa_signature.js getAddressByPhone --phone 18628395611
 *   node scripts/test_rsa_signature.js getPhoneByAddress --address 0xF5C1d985C52aE8cB0Ab7a642E378089eF15D9300
 *
 * 需要安装依赖：
...
// const SERVER_URL = 'http://101.245.89.60:8090';
...
async function sendRequest(endpoint, params) {
    ...
    if (config.needsRsaSignature && config.method === 'POST') {
        const signResult = signRequest(config.method, config.path, body, PRIVATE_KEY_PEM);
...
function parseArgs() {
    ...
    for (let i = 1; i < args.length; i += 2) {
        if (args[i] && args[i].startsWith('--')) {
            ...
            if (value !== undefined) {
                // 如果是地址（以 0x 开头），保持为字符串
                if (value.startsWith('0x')) {
                    params[key] = value;
                }
                // 如果是纯数字字符串（不包含小数点），且不是地址，转换为数字
...
function showHelp() {
    ...
    Object.entries(API_ENDPOINTS).forEach(([name, config]) => {
        ...
        console.log(`    需要RSA签名: ${config.needsRsaSignature && config.method === 'POST' ? '是' : '否'}`);
        console.log('');
    });

    console.log('示例：\n');
    console.log('  # 查询 Mix 余额');
    console.log('  node test_rsa_signature.js getMix --phone 18628395611 --address 0xF5C1d985C52aE8cB0Ab7a642E378089eF15D9300\n');
    console.log('  # 添加 Mix 余额');
    console.log('  node test_rsa_signature.js addMix --phone 18628395611 --address 0xF5C1d985C52aE8cB0Ab7a642E378089eF15D9300 --amount 10.5\n');
    console.log('  # 通过手机号查询地址');
    console.log('  node test_rsa_signature.js getAddressByPhone --phone 18628395611\n');
    console.log('  # 通过地址查询手机号');
    console.log('  node test_rsa_signature.js getPhoneByAddress --address 0xF5C1d985C52aE8cB0Ab7a642E378089eF15D9300\n');
    console.log('  # 绑定手机号和地址');
    console.log('  node test_rsa_signature.js boundUserPhone --phone 18628395611 --address 0xF5C1d985C52aE8cB0Ab7a642E378089eF15D9300\n');
    console.log('  # 确认绑定');
    console.log('  node test_rsa_signature.js confirmBinding --phone 18628395611 --address 0xF5C1d985C52aE8cB0Ab7a642E378089eF15D9300\n');
    console.log('  # 更换手机号');
    console.log('  node test_rsa_signature.js changeUserPhone --phone 18628395611 --newPhone 13900139000 --address 0xF5C1d985C52aE8cB0Ab7a642E378089eF15D9300\n');
    console.log('  # 更换地址');
    console.log('  node test_rsa_signature.js changeUserAddress --phone 18628395611 --address 0xF5C1d985C52aE8cB0Ab7a642E378089eF15D9300 --newAddress 0x9876543210987654321098765432109876543210\n');
}
...
async function main() {
    try {
        ...
        if (!parsed || parsed.endpoint === 'help' || parsed.endpoint === '--help' || parsed.endpoint === '-h') {
            showHelp();
            return;
        }
        ...
        // 验证必需参数
        const missingParams = config.params.filter(param => params[param] === undefined);
