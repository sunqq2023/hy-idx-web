...
const MixBill = () => {
    ...
    async (page: number = 1) => {
      try {
        ...
        const mixChildRecords = await readContract(config, {
          ...
          args: [userAddress, startIndex, pageSize], // 参数：用户地址、起始索引、查询数量
        });
        ...
        const allExchangeIds = bigNumIds as bigint[];
        const paginatedExchangeIds = allExchangeIds.slice(
          startIndex,
          startIndex + pageSize,
        );

        const contracts = paginatedExchangeIds.map((id) => ({
          address: MiningMachineProductionLogicAddress,
          abi: MiningMachineProductionLogicABI,
        ...
        const result =
          contracts.length > 0 ? await multicall(config, { contracts }) : [];
        const exchangeIdxRecordsList = result.map((item: any) => ({
        ...
        const claimMixRecords = await readContract(config, {
          ...
          args: [userAddress, startIndex, pageSize],
        });
        ...
        const list = [
          ...
          const aTime = "timestamp" in a ? a.timestamp : a.startTime;
          const bTime = "timestamp" in b ? b.timestamp : b.startTime;
          return bTime - aTime;
        }); // 按时间倒序排列

        setRecordList(list);
        setTotalRecords(
          mixChildRecordsList.length +
            allExchangeIds.length +
            claimMixRecordsList.length,
        );
      } catch (error) {
  ...
  }, [handleQuery, userAddress, currentPage]);
          ...
          {item.type === 0 && (
            <>
              ...
              <div className="flex gap-2">
                ...
                <div className="text-black ">MIX积分</div>
              </div>
              <div className="flex gap-2">
                <div>矿机合成时间:</div>
        ...
        {!isQuerying ? (
          recordList.length > 0 ? (
            <>
              ...
              <div className="flex justify-center items-center gap-4 py-3 text-[14px]">
                <button
