...
import { formatEther, formatUnits, parseEther, erc20Abi, parseGwei } from 'viem'
import {
...
export const Team = ({}: TeamProps) => {
  ...
  const [introducerAddress, setIntroducerAddress] = useState<string | null>(null) // 推荐人地址
  ...
  const [claimedActivatedMachineRewards, setClaimedActivatedMachineRewards] = useState('0') // 已获得的释放奖励值
  ...
  const checkStudioStatus = useCallback(async () => {
    ...
    try {
      ...
      console.log('工作室状态:', studioStatus, '标记者状态:', studioMarkerStatus)
    } catch (error) {
  ...
  const fetchWithdrawRecords = useCallback(async () => {
    ...
    try {
      ...
      // 转换数据格式
      const formattedRecords = (records as any[]).map((record: any, index: number) => {
  ...
  const fetchAirdropData = useCallback(async () => {
    ...
    try {
      ...
      console.log('getActivatedMachineRewards 结果:', activatedRewards.toString())
      ...
      console.log('getClaimedActivatedMachineRewards 结果:', claimedRewards.toString())
      ...
      console.log('calculateActivatedMachineRewards 结果:', withdrawable.toString())
      ...
      console.log('4. 除法计算 (rateInWei / divisor):', Number(rateInWei) / Number(divisor))
      console.log('5. 转换为百分比 (× 100):', Number(rateInWei) / Number(divisor) * 100)
  ...
  // 递归计算团队总人数
  const calculateTotalTeamCount = useCallback(async (address: string, visited: Set<string> = new Set()): Promise<number> => {
    // 防止循环引用
    ...
    try {
      ...
      for (const friend of friendList) {
        const subTeamCount = await calculateTotalTeamCount(friend, new Set(visited))
  ...
  const fetchTeamData = useCallback(async () => {
    ...
    try {
      ...
      setPerformanceCommission(claimableRewards ? formatEther(claimableRewards) : '0')
  ...
  useEffect(() => {
    ...
    fetchAirdropData()
    checkStudioStatus()
  }, [fetchTeamData, fetchWithdrawRecords, fetchTreasuryBalance, fetchAirdropData, checkStudioStatus])

  // 检查是否可以提现
  const canWithdraw = parseFloat(performanceCommission) > 0 && parseFloat(treasuryBalance) >= parseFloat(performanceCommission)
  
  // 检查是否可以提取空投奖励
  const canWithdrawAirdrop = parseFloat(withdrawableRewards) > 0 && parseFloat(treasuryBalance) >= parseFloat(withdrawableRewards)
  ...
  const handleWithdrawAirdrop = async () => {
    if (!canWithdrawAirdrop) {
      ...
      } else if (parseFloat(treasuryBalance) < parseFloat(withdrawableRewards)) {
        Toast.show({
          content: '奖励池余额不足',
          position: 'center'
        })
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="bg-[#09090a] rounded-2xl text-white p-4 mt-2 relative">
          ...
          <div className="flex items-center justify-between py-4 relative">
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
              ...
              {/* 根据是否有推荐人显示不同内容 */}
              {introducerAddress && introducerAddress !== '0x0000000000000000000000000000000000000000' ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
              ...
              >
                {isWithdrawing ? '提取中...' : 
                 parseFloat(performanceCommission) > 0 && parseFloat(treasuryBalance) < parseFloat(performanceCommission) ? '奖励池不足' : 
                 '提取到钱包'}
              </Button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4">
            ...
            <div className="relative">
              ...
              <div className="flex items-center mb-3">
                ...
                >
                  <span className="text-black text-[12px] font-medium mr-1">每天释放速率?</span>
                  <svg 
              ...
              <div className="flex items-center mb-4">
                <div className="flex-1 text-center text-gray-500 text-[12px]">
                  {isLoadingAirdropData ? (
                    ...
                    `${formatAirdropNumber(activatedMachineRewards)}/${formatAirdropNumber(claimedActivatedMachineRewards)}`
                  )}
                </div>
            ...
            >
              {isWithdrawing ? '提取中...' : 
               parseFloat(withdrawableRewards) > 0 && parseFloat(treasuryBalance) < parseFloat(withdrawableRewards) ? '奖励池不足' : 
               '提取到钱包'}
            </Button>
          </div>
        </div>
      ...
      {/* 提现表单弹窗 */}
      {showWithdrawForm && createPortal(
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center" style={{ zIndex: 99999 }}>
          <div className="bg-white rounded-2xl p-6 mx-4 w-full max-w-sm">
            <div className="text-lg font-bold mb-4 text-center">提现到钱包</div>
            
            <div className="mb-4">
              <div className="text-sm text-gray-600 mb-2">
                可提现金额: <span className="font-bold text-[#895EFE]">{performanceCommission} IDX</span>
              </div>
              {/* <div className="text-sm text-gray-600 mb-2">
      ...
      {/* 推荐人填写表单弹窗 */}
      {showReferrerForm && createPortal(
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center" style={{ zIndex: 99999 }}>
          <div className="bg-white rounded-2xl p-6 mx-4 w-full max-w-sm">
            <div className="text-lg font-bold mb-4 text-center">请输入你的推荐人钱包地址</div>
      ...
      {/* 推荐人确认弹窗 */}
      {showReferrerConfirm && createPortal(
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center" style={{ zIndex: 99999 }}>
          <div className="bg-white rounded-2xl p-6 mx-4 w-full max-w-sm">
            <div className="text-lg font-bold mb-4 text-center">确认提示:</div>
      ...
      {/* 标记工作室表单弹窗 */}
      {showMarkStudioForm && createPortal(
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center" style={{ zIndex: 99999 }}>
          <div className="bg-white rounded-2xl p-6 mx-4 w-full max-w-sm">
            <div className="text-lg font-bold mb-4 text-center">标记工作室</div>
          ...
          <div className="bg-white rounded-2xl p-6 mx-4 w-full max-w-sm max-h-[80vh] flex flex-col">
            ...
            <div className="bg-gray-50 rounded-xl p-4 mb-4">
              ...
              <div className="text-2xl font-bold text-black">
                {powerRecords.reduce((total, record) => total + parseFloat(record.getAmount), 0).toFixed(0)}
              </div>
            </div>
              ...
              {isLoadingPowerRecords ? (
                ...
                <div className="space-y-3">
                  {powerRecords.map((record, index) => (
                    <div key={index} className="bg-white border border-gray-200 rounded-xl p-4 flex items-center">
                      {/* 左侧图标 */}
