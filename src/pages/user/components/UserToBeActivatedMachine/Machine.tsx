...
import { erc20Abi, formatEther, parseEther, TransactionReceipt, parseGwei } from 'viem'
...
const Machine = ({ isShow }: { isShow: boolean }) => {
  ...
  const handleVisibleDataUpdate = useCallback(async () => {
    console.log('handleVisibleDataUpdate 被调用, allMachineIds.length:', allMachineIds.length);
    ...
    try {
      ...
      console.log('矿机详细状态:', updatedMachines.map(m => ({ 
        id: m.id, 
        mtype: m.mtype, 
        isOnSale: m.isOnSale, 
      ...
      console.log('母矿机数量:', motherMachines.length, 'ID:', motherMachines.map(m => m.id));
      console.log('子矿机数量:', childMachines.length, 'ID:', childMachines.map(m => m.id));
      
      // 分析子矿机状态
      const activatedChildren = childMachines.filter(m => m.isActivatedStakedLP);
      const unactivatedChildren = childMachines.filter(m => !m.isActivatedStakedLP);
      const onSaleChildren = childMachines.filter(m => m.isOnSale);
      const notOnSaleChildren = childMachines.filter(m => !m.isOnSale);
      
      console.log('已激活子矿机:', activatedChildren.length, 'ID:', activatedChildren.map(m => m.id));
      console.log('未激活子矿机:', unactivatedChildren.length, 'ID:', unactivatedChildren.map(m => m.id));
      console.log('在售子矿机:', onSaleChildren.length, 'ID:', onSaleChildren.map(m => m.id));
      console.log('未在售子矿机:', notOnSaleChildren.length, 'ID:', notOnSaleChildren.map(m => m.id));
      ...
      const finalChildList = childMachines
        .filter(m => {
          ...
          console.log(`矿机 ${m.id} 筛选条件: isOnSale=${m.isOnSale}(${isNotOnSale}), isActivatedStakedLP=${m.isActivatedStakedLP}(${isNotActivated})`);
      ...
      console.log('应该显示的矿机ID:', finalChildList.map(m => m.id));
      setMachineList(finalChildList);

      // 更新母矿机ID列表
      const activatedMotherMachines = updatedMachines.filter(m => m.mtype === 1 && m.isActivatedStakedLP);
      const claimableMotherMachines = activatedMotherMachines.filter(m => (m.claimableChildren || 0) > 0);
      const mmIds = claimableMotherMachines.map(m => m.id);
      setMMIds(mmIds);

      // 计算可领取子矿机总数
      const totalClaimable = claimableMotherMachines.reduce((sum, m) => sum + (m.claimableChildren || 0), 0);
  ...
  // 单台矿机刷新处理
  const handleSingleMachineRefresh = useCallback(async (machineId: number, newData: any) => {
    try {
      // 更新矿机数据
      setAllMachines(prev => {
        const newMachines = [...prev];
      ...
      if (newData.mtype === 1 && newData.isActivatedStakedLP) {
        setMMIds(prev => {
          ...
          } else if (newData.claimableChildren === 0 && prev.includes(machineId)) {
  ...
  const handleClaimChildren = useCallback(async () => {
    try {
      ...
      selectedMMIds.forEach(id => {
        ...
        if (machine.mtype !== 1) {
          validationResult.invalidIds.push(`ID=${id}（非母矿机，mtype=${machine.mtype}）`);
      ...
      if (validationResult.invalidIds.length > 0) {
        console.error(`发现${validationResult.invalidIds.length}个无效矿机: ${validationResult.invalidIds.join('; ')}`);
        Toast.show({
  ...
  const handleSelectByCount = () => {
    ...
    const activatableMachines = machineList.filter(item => !item.isActivatedStakedLP)
  ...
  const handleActivate = async () => {
    ...
    console.log(`准备激活选中的矿机，共${fuelList.length}台，ID列表:`, fuelList.map(item => item.id))
    setOpen(true)
  }
  ...
  const handlePay = async () => {
    try {
      ...
      setMaskVisible(true);
      setIsPaying(true);
      console.log('开始执行支付流程，检查授权状态:', isAllowanceSufficient ? '已授权' : '未授权')

      // 1. 检查并处理智能授权
      if (!isAllowanceSufficient) {
        console.log('开始执行智能授权检查，授权地址:', MiningMachineSystemLogicAddress)
        ...
        if (currentAllowance >= smartAllowance) {
          ...
          try {
            ...
            if (error.message.includes('gasLimit') || error.message.includes('null')) {
              ...
              console.log('智能授权交易已发送（回退方案），哈希:', approveHash)
            } else {
      ...
      try {
        ...
        if (error.message.includes('gasLimit') || error.message.includes('null')) {
          ...
          hash = await writeContract(config, {
            ...
            functionName: 'batchActivateMachinesWithLP',
            args: [machineIds],
            gas: 200000n, // 批量激活需要更多Gas
            maxFeePerGas: parseGwei('25'), // 稍高的费用确保交易成功
            maxPriorityFeePerGas: parseGwei('3') // 适中的优先费用
          });
          console.log('批量激活交易已发送（回退方案），哈希:', hash)
        } else {
          throw error // 其他错误正常抛出
        }
      }
  ...
  const handleSellOut = async () => {
    ...
    console.log('准备挂售的矿机列表:', fuelList.map(item => item.id))
    ...
    try {
      ...
      const multiContractsCalls = fuelList.map((item, index) => ({
        ...
        onConfirmed: (receipt: TransactionReceipt, callIndex: number) => {
          successCount++
          console.log(`第${callIndex + 1}/${fuelList.length}个挂售调用已确认，区块号: ${receipt.blockNumber}`)
        }
      }))

      console.log(`开始执行挂售批量调用，共${multiContractsCalls.length}个矿机`)
        ...
        {claimChildrenCount > 0 && mmIds.length > 0 && (
          <div className="w-full bg-black border border-gray-600 rounded-3xl px-4 py-5">
            ...
            <div className="flex items-center justify-between text-[15px] text-white mb-3">
              <span>
                <span className="text-red text-[16px] font-bold">{claimChildrenCount}</span>
                个子矿机待领取
              </span>
