...
export function effectiveBalance(
  ...
  if (integer > 0) {
    // 使用四舍五入而不是截断
    const fullNumber = parseFloat(`${integer}.${decimal}`);
    const rounded = fullNumber.toFixed(decimalSubLen);
    const [roundedInt, roundedDec] = rounded.split(".");
    const str = roundedDec || "00";
    const res = `${roundedInt}.${str}`;
    return Number(res) > 1000
      ? `${Number(roundedInt).toLocaleString()}.${str}`
      : res;
  }
